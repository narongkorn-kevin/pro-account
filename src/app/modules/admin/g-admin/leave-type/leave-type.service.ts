...
*/
  ...
  createItemType(itemtype: any): Observable<any> {
    ...
    //     return throwError('User is already logged in.');
    // }
    return this._httpClient.post(environment.API_URL + 'api/leave_type', itemtype, this.httpOptionsFormdata).pipe(
      switchMap((response: any) => {
        // Return a new observable with the response
        return of(response);
      })
  ...
  getItemType(): Observable<any> {
    return this._httpClient.get<any>(environment.API_URL + 'api/get_leave_type');
  }

  //   * get branch by id
  getItemTypeById(itemtypeId: string): Observable<any[]> {
    return this._httpClient.get<any[]>(environment.API_URL + 'api/leave_type/' + itemtypeId).pipe(
      tap((meterial) => {
        this._materials.next(meterial);
      })
    );
  ...
  updateItemType(itemtype: any, itemtypeId): Observable<any> {
    return this._httpClient.put(environment.API_URL + 'api/leave_type/' + itemtypeId, itemtype, this.httpOptionsFormdata).pipe(
      switchMap((response: any) => {
        // Return a new observable with the response
        return of(response);
      })
  ...
  getItemTypePage(dataTablesParameters: any): Observable<DataTablesResponse> {
    return this._httpClient.post(environment.API_URL + 'api/leave_type_page', dataTablesParameters, this.httpOptionsFormdata).pipe(
      switchMap((response: any) => {
  ...
  delete(itemId: number): Observable<{}> {
    return this._httpClient
      .delete<any>(`${environment.API_URL}api/leave_type/${itemId}`, this.httpOptionsFormdata)
      .pipe(
        map((mtplan) => {
          return mtplan;
        }),
